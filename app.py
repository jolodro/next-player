...
def main(page: ft.Page):
    global indice_da_musica, indice_da_musica_max, auto_next_on_off, loop, alt, DIRETORIO_CAPA_PRINCIPAL, DIRETORIO_CACHE_MUSICAS, DIRETORIO_MUSICAS, DIRETORIO_CAPA_PRINCIPAL_ICO, musicas, musicas_com_diretorio
    page.bgcolor = ft.colors.BLACK
    page.title = 'Next Player'
    page.padding = 10
    page.window_min_width = 1155
    ...
    try:
        #print(musicas, musicas_com_diretorio)
        if musicas == []:
            page.window_destroy()
            page.update()
        indice_da_musica = 0
    ...
    dlg_inf = ft.AlertDialog(
        title=ft.Text('Informações Musicas:')
    )

    def open_dlg_inf(e):
        audio = EasyID3(f'{DIRETORIO_MUSICAS}/{musicas[e.control.data]}')
        track_info = {
            'Titulo': audio['title'][0] if 'title' in audio else 'Não Encontrado',
            'Artista(s)': ', '.join(audio['artist']) if 'artist' in audio else 'Não Encontrado',
            'Album': audio['album'][0] if 'album' in audio else 'Não Encontrado',
            'Data de Lancamento': audio['date'][0] if 'date' in audio else 'Não Encontrado',
            'Genero': ', '.join(audio['genre']) if 'genre' in audio else 'Não Encontrado'
        }
        dlg_inf.content = ft.Text(
            f'Título: {track_info['Titulo']}\nArtista(s): {track_info['Artista(s)']}\nÁlbum: {track_info['Album']}\nData de Lançamento: {track_info['Data de Lancamento']}'
        )
        page.dialog = dlg_inf
        dlg_inf.open = True
        page.update()
    ...
    def controles_nt(e):
        global musicas, musicas_com_diretorio, indice_da_musica
        if e.control.selected == False:
            e.control.selected = True
            e.control.update()
            musicas, musicas_com_diretorio = verificador_de_musicas.listar_musicas(DIRETORIO_MUSICAS, True)
            indice_da_musica = musicas.index(musicas[indice_da_musica])
            carregamento_de_musicas(musicas, lista_musicas)
        else:
            e.control.selected = False
            e.control.update()
            musicas, musicas_com_diretorio = verificador_de_musicas.listar_musicas(DIRETORIO_MUSICAS, False)
            indice_da_musica = musicas.index(musicas[indice_da_musica])
            carregamento_de_musicas(musicas, lista_musicas)
        lista_musicas.update()

    controles_n = ft.Container(
        content=ft.ListView([
            ft.IconButton(
                icon=ft.icons.EXPAND_MORE_ROUNDED,
                icon_color=ft.colors.BLUE_200,
                selected_icon=ft.icons.EXPAND_LESS_ROUNDED,
                on_click=controles_nt
            )
        ],
        ),
    ...
    def carregamento_de_musicas(musicas, lista_musicas):
        ...
        for i in range(len(musicas)):
            ...
            lista_musicas.controls.append(ft.Divider())
        #lista_musicas.update()
    carregamento_de_musicas(musicas, lista_musicas)
    lista_musicas.update()
